...
export default function Footer() {
  ...
  async function loadFooterSettings() {
    try {
      ...
      const data = snap.data() as FooterSettings | undefined;
      
      if (data) {
        setSettings({
          footerText: data.footerText || "© 2024 Blog. All rights reserved.",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex flex-col items-center justify-between space-y-6 md:flex-row md:space-y-0">
          ...
          <div className="flex flex-wrap justify-center gap-6">
            ...
            <Link href="/tags" className="text-gray-600 hover:text-blue-600 transition-colors text-sm">
              Tags
            </Link>
            <Link href="/feed.xml" className="text-gray-600 hover:text-blue-600 transition-colors text-sm">
              RSS Feed
            </Link>
          </div>

          {/* Right side - Social Links */}
          {settings.showSocialLinks && enabledSocialLinks.length > 0 && (
